...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    if data_name == "strategyqa":
        # Extract the very first alphabetical letter
        import re
        match = re.search(r'[a-zA-Z]', pred_str)
        if match:
            pred = match.group(0).upper()
            if pred in ['Y', 'N']:
                return pred
        return ""
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", "minerva_math"]:
        ...
        gt_cot = None
        gt_ans = "Y" if example.get("answer", False) else "N"
    else:
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        question = f"{example['question'].strip()}\nPlease start your answer with either YES or NO."
    else:
    ...
    if isinstance(gt_ans, str):
        ...
        if gt_lower in ["yes", "no"] and data_name != "strategyqa":
            question += " (Yes or No)"
